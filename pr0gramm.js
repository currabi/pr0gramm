...
*/
/* Amalgamation 2014-11-03 19:57:17 */
"use strict";
window.p = {
    NAVIGATE: {
        DEFAULT: 0,
    ...
    compileTemplate: function (t) {
        ...
        try {
            ...
            var compiled = new Function("data", "try {" + "data = data || {};" + "var _parts = [];" + "var print = function(a){ _parts.push(a); };" + "with(data){ _parts.push('" + t.replace(re.escapeBackslashes, "\\\\").replace(re.escapeQuotes, "\\'").replace(re.removeWhitspace, " ").replace(re.printEscaped, function (m, js) {
...
Date.MONTH_NAMES = ["Jan", "Feb", "MÃ¤r", "Apr", "Mai", "Jun", "Jul", "Aug", "Sep", "Okt", "Nov", "Dez"];
Date.prototype.readableTime = function () {
    var h = this.getHours();
    var m = this.getMinutes();
    return this.getDate() + ". " + Date.MONTH_NAMES[this.getMonth()] + " " + this.getFullYear() + " - " + (h > 9 ? h : '0' + h) + ":" + (m > 9 ? m : '0' + m);
};
Date.prototype.readableDate = function () {
    return this.getDate() + ". " + Date.MONTH_NAMES[this.getMonth()] + " " + this.getFullYear();
};
"use strict"
...
(function (window) {
    ...
    window.CONFIG = {
        ...
        PATH: {
            IMAGES: (useSubdomains ? 'http://img.' + host + '/' : '/data/images/'),
            FULLSIZE: (useSubdomains ? 'http://full.' + host + '/' : '/data/fullsize/'),
            THUMBS: (useSubdomains ? 'http://thumb.' + host + '/' : '/data/thumbs/')
        },
        ANALYTICS: {
            ENABLED: true,
            ACCOUNT: 'UA-571256-3'
        ...
        TAGS_INPUT_SETTINGS: {
            'defaultText': 'Tags hinzufÃ¼gen, mit Komma trennen',
            'removeWithBackspace': true,
            'minChars': 2,
            'maxChars': 32
        },
        BOOKMARKLET: "javascript:" + "void((function(){" + "var%20e=document.createElement('script');" + "var%20p=(document.location.protocol=='https:'?'https://ssl.':'http://');" + "e.src=p+'" + host + "/media/bookmarklet.min.js';" + "document.body.appendChild(e);" + "})());",
        ADS: {
            ACCOUNT: '58441731',
...
p.ads = {
    ...
    fill: function (elements) {
        ...
        elements.each(function () {
            var $div = $(this);
            var id = $div.attr('id');
            var slotName = '/' + CONFIG.ADS.ACCOUNT + '/' + $div.data('slot') + (p.user.flags == CONFIG.SFW_FLAG.SFW.flag ? '' : '-nsfw')
...
function () {
    ...
    function () {
        ...
        function d() {
            ...
            var b = "Promise" in a && "resolve" in a.Promise && "reject" in a.Promise && "all" in a.Promise && "race" in a.Promise &&
            function () {
                var b;
    ...
    function a(a) {
        ...
        return new m(function (a, d) {
            var e = n.open(c.name, c.version);
            e.onerror = function () {
                d(e.error)
    ...
    function d(a, b, c) {
        ...
        var e = new m(function (c, e) {
            d.ready().then(function () {
                var f = d._dbInfo,
                    g = f.db.transaction(f.storeName, "readwrite").objectStore(f.storeName);
                null === b && (b = void 0);
                var h = g.put(b, a);
    ...
    function e(a, b) {
        ...
        var d = new m(function (b, d) {
            c.ready().then(function () {
                var e = c._dbInfo,
                    f = e.db.transaction(e.storeName, "readwrite").objectStore(e.storeName),
                    g = f["delete"](a);
                g.onsuccess = function () {
                    b()
                }, g.onerror = function () {
        ...
        var b = this,
            c = new m(function (a, c) {
                b.ready().then(function () {
                    var d = b._dbInfo,
                        e = d.db.transaction(d.storeName, "readwrite").objectStore(d.storeName),
                        f = e.clear();
    ...
    var m = "undefined" != typeof module && module.exports ? require("promise") : this.Promise,
        n = n || this.indexedDB || this.webkitIndexedDB || this.mozIndexedDB || this.OIndexedDB || this.msIndexedDB;
    if (n) {
        var o = {
        ...
        "function" == typeof define && define.amd ? define("asyncStorage", function () {
            return o
        }) : "undefined" != typeof module && module.exports ? module.exports = o : this.asyncStorage = o
    }
    ...
    function a(a) {
        var b = this,
            c = {};
        if (a) for (var d in a) c[d] = a[d];
        return c.keyPrefix = c.name + "/", b._dbInfo = c, n.resolve()
    }
    function b(a) {
        var b = this,
            c = new n(function (a, c) {
                b.ready().then(function () {
                    for (var c = b._dbInfo.keyPrefix, d = o.length - 1; d >= 0; d--) {
                        var e = o.key(d);
                        0 === e.indexOf(c) && o.removeItem(e)
                    }
                    a()
                })["catch"](c)
            });
    ...
    function d(a, b) {
        var c = this,
            d = new n(function (b, d) {
                c.ready().then(function () {
                    try {
                        for (var e = c._dbInfo.keyPrefix, f = e.length, g = o.length, h = 0; g > h; h++) {
                            var j = o.key(h),
                                k = o.getItem(j);
                            if (k && (k = i(k)), k = a(k, j.substring(f)), void 0 !== k) return void b(k)
                        }
                        b()
                    } catch (l) {
                        d(l)
        ...
        var c = this,
            d = new n(function (b, d) {
                c.ready().then(function () {
                    ...
                    d && (d = d.substring(e.keyPrefix.length)), b(d)
                })["catch"](d)
            });
        return m(d, b), d
    }
    function f(a) {
        var b = this,
            c = new n(function (a, c) {
                b.ready().then(function () {
                    for (var c = b._dbInfo, d = o.length, e = [], f = 0; d > f; f++) 0 === o.key(f).indexOf(c.keyPrefix) && e.push(o.key(f).substring(c.keyPrefix.length));
                    a(e)
                })["catch"](c)
            });
        return m(c, a), c
    ...
    function h(a, b) {
        ...
        var d = new n(function (b, d) {
            c.ready().then(function () {
                var d = c._dbInfo;
                o.removeItem(d.keyPrefix + a), b()
            })["catch"](d)
        });
        return m(d, b), d
    }
    function i(a) {
        if (a.substring(0, r) !== q) return JSON.parse(a);
        for (var b = a.substring(D), c = a.substring(r, D), d = new ArrayBuffer(2 * b.length), e = new Uint16Array(d), f = b.length - 1; f >= 0; f--) e[f] = b.charCodeAt(f);
        switch (c) {
        case s:
            return d;
        case t:
    ...
    function j(a) {
        ...
        try {
            ...
            for (var e = 0; e < c.length; e++) b += String.fromCharCode(c[e])
        }
        return b
    }
    function k(a, b) {
        ...
        if (a && (c = a.toString()), a && ("[object ArrayBuffer]" === a.toString() || a.buffer && "[object ArrayBuffer]" === a.buffer.toString())) {
            ...
            window.console.error("Couldn't convert value into a JSON string: ", a), b(g)
        }
    }
    function l(a, b, c) {
        var d = this;
            ...
            d.ready().then(function () {
                ...
                k(b, function (b, g) {
                    if (g) e(g);
                        ...
                        try {
                            var h = d._dbInfo;
                            o.setItem(h.keyPrefix + a, b)
                        } catch (i) {
                            ("QuotaExceededError" === i.name || "NS_ERROR_DOM_QUOTA_REACHED" === i.name) && e(i)
                        }
                        c(f)
                    }
                })
    ...
    var n = "undefined" != typeof module && module.exports ? require("promise") : this.Promise,
        o = null;
    try {
    ...
    "function" == typeof define && define.amd ? define("localStorageWrapper", function () {
        return E
    }) : "undefined" != typeof module && module.exports ? module.exports = E : this.localStorageWrapper = E
}.call(window), function () {
    "use strict";
    ...
    function a(a) {
        ...
        return new o(function (d, e) {
            try {
                ...
                return b.setDriver("localStorageWrapper").then(function () {
                    ...
                    e.db.transaction(function (c) {
                        c.executeSql("SELECT * FROM " + e.storeName, [], function (c, d) {
                            for (var e = d.rows, f = e.length, g = 0; f > g; g++) {
                                var h = e.item(g),
                                    i = h.value;
                                if (i && (i = k(i)), i = a(i, h.key), void 0 !== i) return void b(i)
                            }
                            b()
                        }, function (a, b) {
                            d(b)
    ...
    function j(a) {
        var b, c = new Uint8Array(a),
            d = "";
        for (b = 0; b < c.length; b += 3) d += n[c[b] >> 2], d += n[(3 & c[b]) << 4 | c[b + 1] >> 4], d += n[(15 & c[b + 1]) << 2 | c[b + 2] >> 6], d += n[63 & c[b + 2]];
        return c.length % 3 === 2 ? d = d.substring(0, d.length - 1) + "=" : c.length % 3 === 1 && (d = d.substring(0, d.length - 2) + "=="), d
    }
    function k(a) {
        if (a.substring(0, r) !== q) return JSON.parse(a);
        var b, c, d, e, f, g = a.substring(D),
            h = a.substring(r, D),
            i = .75 * g.length,
            j = g.length,
            k = 0;
        ...
        var l = new ArrayBuffer(i),
            m = new Uint8Array(l);
        for (b = 0; j > b; b += 4) c = n.indexOf(g[b]), d = n.indexOf(g[b + 1]), e = n.indexOf(g[b + 2]), f = n.indexOf(g[b + 3]), m[k++] = c << 2 | d >> 4, m[k++] = (15 & d) << 4 | e >> 2, m[k++] = (3 & e) << 6 | 63 & f;
        switch (h) {
        case s:
            return l;
        case t:
    ...
    function l(a, b) {
        ...
        if (a && (c = a.toString()), a && ("[object ArrayBuffer]" === a.toString() || a.buffer && "[object ArrayBuffer]" === a.buffer.toString())) {
            ...
            window.console.error("Couldn't convert value into a JSON string: ", a), b(null, g)
        }
    }
    function m(a, b) {
        b && a.then(function (a) {
    ...
    var n = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",
        o = "undefined" != typeof module && module.exports ? require("promise") : this.Promise,
        p = this.openDatabase,
        q = "__lfsc__:",
        r = q.length,
        s = "arbf",
    ...
    if (p) {
        ...
        "function" == typeof define && define.amd ? define("webSQLStorage", function () {
            return E
        }) : "undefined" != typeof module && module.exports ? module.exports = E : this.webSQLStorage = E
    }
}.call(window), function () {
    "use strict";
    ...
    var e = "undefined" != typeof module && module.exports ? require("promise") : this.Promise,
        ...
        l = j.WINDOW;
    "function" == typeof define && define.amd ? l = j.DEFINE : "undefined" != typeof module && module.exports && (l = j.EXPORT);
    var m = function (a) {
            var b = b || a.indexedDB || a.webkitIndexedDB || a.mozIndexedDB || a.OIndexedDB || a.msIndexedDB,
                c = {};
            return c[g.WEBSQL] = !! a.openDatabase, c[g.INDEXEDDB] = !!
            function () {
...
p.VoteCache = p.Class.extend({
    ...
    _cachedLastSyncId: -1,
    _lastSyncKey: 'last_sync_0',
    setName: function (name) {
        if (typeof name === 'string') {
            this._prefix = name.replace(/[^\w]/, '') + '_';
        }
        localforage.getItem('lastSyncId', this.cleanupOldDB.bind(this));
    },
    cleanupOldDB: function (error, needsCleanup) {
        if (!needsCleanup) {
            return;
        }
        var oldKeys = ['lastSyncId', 'items_up', 'items_down', 'comments_up', 'comments_down', 'tags_up', 'tags_down'];
        for (var i = 0; i < oldKeys.length; i++) {
            localforage.removeItem(oldKeys[i]);
        }
    },
    clear: function () {
        this.votes.items = {};
    ...
    getLastSyncId: function (callback) {
        if (!this._prefix) {
            return;
        }
        var that = this;
        localforage.getItem(this._prefix + this._lastSyncKey, function (error, id) {
            if (error) {
                return;
            }
    ...
    restoreFromLocal: function (callback) {
        ...
        for (var thing in this.votes) {
            ...
            localforage.getItem(this._prefix + thing + '_up', this.combineVotes.bind(this, thing, 1));
            localforage.getItem(this._prefix + thing + '_down', this.combineVotes.bind(this, thing, -1));
            localforage.getItem(this._prefix + thing + '_fav', this.combineVotes.bind(this, thing, 2));
        }
    },
    ...
    saveToLocal: function () {
        ...
        for (var thing in this.votes) {
            ...
            for (var action in splitted) {
                ...
                if (ids.length) {
                    localforage.setItem(this._prefix + thing + '_' + action, ids);
                }
            }
        }
    },
    ...
    _updateFromLogInternal: function (log, lastId, callback) {
        localforage.setItem(this._prefix + this._lastSyncKey, lastId);
...
p.User = p.Class.extend({
    ...
    loadCookie: function () {
        ...
        if (restored) {
            ...
            if (this.id) {
                ...
                this.voteCache.setName(this.name);
    ...
    sync: function () {
        var callback = this.syncCallback.bind(this);
        this.voteCache.getLastSyncId(function (lastSyncId) {
            p.api.get('user.sync', {
                lastId: lastSyncId
            }, callback);
        });
    ...
    restoreVotes: function () {
        ...
        p.api.get('user.sync', {
            lastId: 0
        }, this.syncCallback.bind(this));
    },
    setInboxLink: function (count) {
        this.inboxCount = count;
        var empty = (this.inboxCount == 0);
        document.title = empty ? 'pr0gramm.com' : '[' + count + '] pr0gramm.com';
...
p.adjustVote = function (thing, vote) {
    if (!thing) {
        return;
    }
    if (thing.vote < 0) {
...
p.View.Base = p.Class.extend({
    ...
    render: function () {
        ...
        anchors.click(this.handleHashLink.bind(this));
...
p.View.Base.LoadingAnimHTML = $('<div class="spinner">' + '<div class="bounce1"></div>' + '<div class="bounce2"></div>' + '<div class="bounce3"></div>' + '</div>');
...
p.View.Overlay.Settings = p.View.Base.extend({
    name: 'overlay-settings',
    template: '<div class="overlay-content"> <h2>Einstellungen</h2> <span class="action" id="settings-logout-link">abmelden</span> </div> <div class="overlay-tabs"> <span class="link tab" id="settings-bookmarklet">Bookmarklet</span> <span class="link tab" id="settings-profile">Profil</span> <span class="link tab" id="settings-account">Account</span> </div> <div class="overlay-content"> <?js if(saved) {?> <h2>Gespeichert!</h2> <?js } else { ?> <?js if(tab == \'bookmarklet\') { ?> <div class="bookmarklet-frame"> <p> Mit dem <em>+pr0g</em> Bookmarklet kannst du direkt von jeder Website Bilder auf pr0gramm hochladen. </p> <p> <a class="bookmarklet" title="Ziehe diesen Link in deine Bookmarks!" href="{CONFIG.BOOKMARKLET}"> <img src="/media/pr0gramm-favicon.png" class="bookmarklet-icon"/> +pr0g </a> &nbsp;&laquo; Ziehe diesen Link in deine Bookmarks! </p> <img src="/media/bookmarklet-instruction.gif" class="bookmarklet-instruction"/> </div> <?js } else if( tab == \'profile\') { ?> <form method="post" id="settings-profile-form"> <input type="checkbox" name="likesArePublic" id="likesArePublic" value="1" <?js if(account.likesArePublic) {?>checked="checked"<?js } ?> /> <label for="likesArePublic" class="checkbox-label"> Favoriten sind fÃ¼r jeden sichtbar </label> <div class="form-row"> <input type="submit" value="Speichern" class="settings-save"/> </div> </form> <?js } else if( tab == \'account\') { ?> <form method="post" id="settings-account-form"> <div class="form-row"> <label>E-Mail Adresse</label> <input type="email" name="email" value="{{account.email}}" placeholder="E-Mail Adresse"/> <?js if( error === \'emailInUse\' ) { ?> <p class="warn">Diese E-Mail Adresse ist schon in Verwendung</p> <?js } else if( error === \'emailInvalid\' ) { ?> <p class="warn">UngÃ¼ltige E-Mail Adresse</p> <?js } ?> <p class="field-details"></p> </div> <div class="form-row"> <label>Neues Passwort<br/> (Feld leer lassen, wenn du es nicht Ã¤ndern mÃ¶chtest)</label> <input type="password" name="password" placeholder="Neues Passwort"/> <?js if( error === \'passwordTooShort\' ) { ?> <p class="warn">Passwort zu kurz</p> <?js } ?> <p class="field-details">Mindestens 6 Zeichen</p> </div> <div class="form-row"> <label>Derzeitiges Passwort</label> <input type="password" name="currentPassword" placeholder="Passwort"/> <?js if( error === \'passwordInvalid\' ) { ?> <p class="warn">Passwort ungÃ¼ltig</p> <?js } ?> </div> <div class="form-row"> <input type="submit" value="Speichern" class="settings-save"/> </div> <?js } ?> <?js } ?> </div> ',
    data: {
        error: null,
        saved: false,
        tab: 'bookmarklet',
    ...
    load: function () {
        p.api.get('user.me', {}, this.loaded.bind(this));
        return false;
    },
    ...
    render: function () {
        this.parent();
        $('#settings-logout-link').click(this.logout.bind(this));
        this.$container.find('.bookmarklet').click(this.bookmarkClick.bind(this));
        var tabs = this.$container.find('.tab');
        tabs.click(this.switchTab.bind(this));
        tabs.filter('#settings-' + this.data.tab).addClass('active');
        var that = this;
        this.$container.find('#settings-profile-form').submit(function (ev) {
            p.api.post('user.profile', ev.currentTarget, that.posted.bind(that));
            that.renderLoading('#settings-profile-form');
            return false;
        });
        this.$container.find('#settings-account-form').submit(function (ev) {
            p.api.post('user.account', ev.currentTarget, that.posted.bind(that));
            that.renderLoading('#settings-account-form');
            return false;
        });
    ...
    switchTab: function (ev) {
        var $a = $(ev.target);
        this.data.tab = $a.attr('id').split('-')[1];
    ...
    logout: function (ev) {
        p.user.logout();
        p.mainView.unsetUserStatus();
        p.navigateTo('', p.NAVIGATE.FORCE);
        return false;
    }
});
p.View.Overlay = p.View.Overlay || {};
p.View.Overlay.SetEmail = p.View.Base.extend({
    name: 'overlay-set-email',
    template: '<div class="overlay-content"> <h2>Account reaktivieren</h2> </div> <div class="overlay-content"> <?js if( sent ) { ?> <p>Danke! Du solltest gleich eine E-Mail mit dem Aktivierungslink bekommen.</p> <?js } else { ?> <form id="set-email-form"> <p> Willkommen zurÃ¼ck! Wir brauchen nur noch deine E-Mail Adresse, um dir einen Aktivierungslink zu schicken. </p> <?js if( error === \'emailInvalid\' ) { ?> <p class="warn">UngÃ¼ltige E-Mail Adresse</p> <?js } else if( error === \'emailInUse\' ) { ?> <p class="warn">Diese E-Mail Adresse wird bereits verwendet</p> <?js } ?> <div class="form-row"> <input type="email" name="email" placeholder="E-Mail Adresse" value=""/> <input type="hidden" name="token" value="{{token}}"/> </div> <div class="form-row"> <input type="submit" value="Abschicken"/> </div> </form> <?js } ?> </div> ',
    data: {
        error: null,
        sent: false
    },
    init: function (container, parent, token) {
        this.data.token = token;
        this.parent(container, parent);
    },
    render: function () {
        this.parent();
        this.$container.find('#set-email-form').submit(this.submit.bind(this));
        this.focus('input[name=email]');
    },
    submit: function (ev) {
        p.api.post('user.setemail', ev.target, this.posted.bind(this));
        this.renderLoading();
        return false;
    },
    posted: function (response) {
        this.data.error = response.error;
        this.data.sent = !response.error;
        this.render();
    }
});
p.View.Overlay = p.View.Overlay || {};
p.View.Overlay.Login = p.View.Base.extend({
    name: 'overlay-login',
    template: '<div class="overlay-content"> <h2>Anmelden</h2> </div> <div class="overlay-content"> <?js if(loginFailed) {?> <?js if(ban && ban.banned) { ?> <p class="warn"> Dein Account ist gesperrt. <?js if(ban.till) {?> Die Sperrung wird am {ban.till.readableTime()} ({ban.till.relativeTime()}) aufgehoben. <?js } else { ?> Die Sperrung ist dauerhaft. <?js } ?> <?js if(ban.reason) {?> <strong>Grund:</strong> {{ban.reason}} <?js } ?> </p> <?js } else { ?> <p class="warn">Falscher Benutzername oder Passwort</p> <?js } ?> <?js } ?> <form> <div class="form-row"> <input type="text" name="name" placeholder="Name" tabindex="201" value="{name}"/> </div> <div class="form-row"> <input type="password" name="password" placeholder="Passwort" tabindex="202"/> </div> <div class="form-row"> <input type="submit" value="Anmelden" id="login-button" tabindex="203"/> </div> <p> <span class="link" id="reset-password-link">Passwort vergessen?</span> </p> <p> <span class="link" id="login-register-link">Noch keinen Account? Jetzt registrieren!</span> </p> </form> </div> ',
    data: {
        ban: null,
        name: '',
        loginFailed: false
    ...
    render: function () {
        ...
        $('#login-register-link').click(p.mainView.showRegister.bind(p.mainView));
    },
    submit: function (ev) {
    ...
    posted: function (response) {
        if (response.success) {
            p.reload();
        } else if (response.needsEmail) {
            p.mainView.showOverlay(p.View.Overlay.SetEmail, response.emailToken);
        } else {
...
p.View.Overlay = p.View.Overlay || {};
p.View.Overlay.Register = p.View.Base.extend({
    name: 'overlay-register',
    template: '<div class="overlay-content"> <h2>Registrieren</h2> </div> <div class="overlay-content"> <?js if( registered ) { ?> <h3>Danke!</h3> <p> Du solltest gleich eine E-Mail mit dem Aktivierungslink bekommen. <span class="warn">Bitte schau auch in den Spam-Ordner!</span> </p> <p> Solltest du deinen Account nicht innerhalb von 24 Stunden aktivieren, wird er gelÃ¶scht und der Name und E-Mail Adresse wieder freigegeben. </p> <?js } else { ?> <form method="post" id="register-form"> <div class="form-row"> <input type="text" name="name" value="{name}" placeholder="Name"/> <?js if( error === \'nameInvalid\' ) { ?> <p class="warn">UngÃ¼ltiger Name</p> <?js } ?> <?js if( error === \'nameInUse\' ) { ?> <p class="warn">Dieser Name wird bereits verwendet</p> <?js } ?> <p class="field-details">Mindestens 2 Zeichen, nur Buchstaben und Zahlen</p> </div> <div class="form-row"> <input type="email" name="email" value="{email}" placeholder="E-Mail Adresse"/> <?js if( error === \'emailInUse\' ) { ?> <p class="warn">Diese E-Mail Adresse wird bereits verwendet</p> <?js } else if( error === \'emailInvalid\' ) { ?> <p class="warn">Die E-Mail Adresse ist ungÃ¼ltig</p> <?js } ?> <p class="field-details"> Der Aktivierungslink wird an diese Adresse gesendet. Bei einigen Mail-Hostern landet die Mail im Spam-Ordner oder kommt garnicht an. Wir empfehlen <a href="https://mail.google.com/" target="_blank">Google Mail</a>. </p> </div> <div class="form-row"> <input type="password" name="password" placeholder="Passwort"/> <?js if( error === \'passwordTooShort\' ) { ?> <p class="warn">Passwort zu kurz</p> <?js } ?> <p class="field-details">Mindestens 6 Zeichen</p> </div> <div class="form-row"> <input type="submit" value="Registrieren"/> </div> </form> <?js } ?> </div>',
    data: {
        error: null,
        name: '',
        email: '',
    ...
    submit: function (ev) {
        ...
        this.data.error = null;
        p.api.post('user.join', ev.target, this.posted.bind(this));
        this.renderLoading();
        return false;
    },
    posted: function (response) {
...
p.View.Overlay.ResetPassword = p.View.Base.extend({
    ...
    template: '<div class="overlay-content"> <h2>Passwort zurÃ¼cksetzen</h2> </div> <div class="overlay-content"> <?js if(resetSuccess) { ?> <div class="notice"> Die E-Mail mit dem Link zum ZurÃ¼cksetzen wurde versendet. </div> <?js } else { ?> <form method="post"> <div class="form-row"> <input type="email" name="email" placeholder="E-Mail Adresse" value=""/> </div> <?js if(resetFailed) {?> <p class="warn">E-Mail Adresse nicht bekannt, oder der Account ist gesperrt</p> <?js } ?> <div class="form-row"> <input type="submit" value="Abschicken"/> </div> </form> <?js } ?> </div> ',
    data: {
        resetFailed: false,
        resetSuccess: false
    },
...
p.View.Rules = p.View.Base.extend({
    template: '<h3>DAS KLEINGEDRUCKTE &ndash; LIES ES!</h3> <ol class="fine-print"> <li>nsfw/nsfl Bilder mÃ¼ssen vor dem Upload entsprechend markiert werden!</li> <li>Keine suggestiven Bilder oder Gore von/mit MinderjÃ¤hrigen/Babies/FÃ¶ten.</li> <li>Keine Tierpornos.</li> <li>Kein stumpfer Rassismus, kein rechtes Gedankengut, keine Nazi-Nostalgie.</li> <li>Keine Werbung; keine Affiliate Links in den Bildern; kein Spam.</li> <li>Keine Informationen oder Bilder von Privatpersonen; Keine Klarnamen in den Uploads, Tags oder Kommentaren.</li> <li>Ein MindestmaÃŸ an BildqualitÃ¤t wird erwartet. Bildmaterial mit starken Kompressionsartefakten, Ã¼bermÃ¤ÃŸig groÃŸen Watermarks oder unsinnig beschnittene/skalierte Bilder werden gelÃ¶scht.</li> <li>Keine Bilder mit Ã¤hnlichem Inhalt in Reihe. ZugehÃ¶riger Content kann in den Kommentaren verlinkt werden.</li> <li>Kommentare wie <em>â€œTag deinen ScheiÃŸâ€</em> und Ã¤hnliches gehÃ¶ren nicht in die Tags.</li> <li>Downvote-Spam, Vote-Manipulation und Tag-Vandalismus werden nicht geduldet.</li> <li>Pro Benutzer ist nur ein Account erlaubt. Indizien fÃ¼r Multiaccounts sind gegenseitige Upvotes oder Spamaktionen.</li> </ol> <p class="warn"> UnfÃ¤higkeit, diese Regeln zu befolgen, wird mit einer Sperrung Deines Accounts honoriert. Diskussionen um die Sperrung werden die Sperrung verlÃ¤ngern. </p> '
});
p.View.UploadSimilar = p.View.Base.extend({
    template: '<p class="warn"> Wir haben ein paar Ã¤hnliche Bilder gefunden. Bist Du sicher, dass es sich nicht um einen Repost handelt? </p> <div class="upload-similar-images"> <?js for( var i = 0; i < similar.length; i++ ) { var item = similar[i]; ?> <a href="/new/{item.id}" target="_blank" class="similar-thumb"><img src="{item.thumb}"/></a> <?js } ?> </div> ',
...
p.View.Overlay = p.View.Overlay || {};
p.View.Overlay.Upload = p.View.Base.extend({
    name: 'overlay-upload',
    template: '<div class="overlay-content"> <h2>Bild hochladen</h2> </div> <div class="overlay-content"> <?js if( !supportsFileUpload ) { ?> <h2 class="warn"> Datei-Upload wird nicht unterstÃ¼zt. Vielleicht solltest du mal Ã¼berlegen nicht so einen Unterschichten-Brauser zu benutzen. </h2> <?js } else { ?> <?js if( error ) {?> <p class="warn"> <?js if( error === \'invalidType\' ) {?> UngÃ¼ltiger Dateityp. Nur JPEG, PNG, GIF oder WebM erlaubt. <?js } else if( error === \'invalid\' ) {?> Bild ist ungÃ¼ltig oder zu groÃŸ (16 Mega Pixel, 4MB max â€“ WebM nur mit VP8 codec, 120sek max, kein Audio) <?js } else if( error === \'blacklisted\' ) {?> Bild oder Domain gesperrt. <?js } else if( error === \'internal\' ) {?> Interner Fehler bei der Verarbeitung. <?js } else if( error === \'download\' ) {?> Download fehlgeschlagen (4MB max). <?js } else if( error === \'missingData\' ) {?> Kein Bild oder URL angegeben. <?js } ?> </p> <?js } ?> <form id="upload-form"> <div id="upload-source"> <div id="upload-droparea"> Bild auswÃ¤hlen <?js if( supportsDragDrop ) { ?> oder per Drag &amp; Drop reinziehen <?js } ?> </div> <div> <h3 class="upload-enter-url">oder URL angeben</h3> <input type="text" name="imageUrl" id="upload-url" placeholder="http://" style="width: 100%;" /> <input type="file" name="image" id="upload-file"/> </div> </div> <div id="upload-progress"> <div id="upload-preview"></div> <div class="progress-bar"> <div class="progress"></div> </div> </div> <div class="upload-tag-container"> <div class="sfw-status"> <label title="Safe for Work"> <input type="radio" name="sfwstatus" value="sfw"> sfw </label> <label title="Not Safe for Work"> <input type="radio" name="sfwstatus" value="nsfw"> nsfw </label> <label title="Not Safe for Life"> <input type="radio" name="sfwstatus" value="nsfl"> nsfl </label> </div> <h3>SFW Status und Tags</h3> <input type="text" class="upload-tagsinput" name="tags"/> </div> <div class="upload-similar"></div> <div> <input type="hidden" name="checkSimilar" value="1"/> <input type="submit" class="button" value="Bild Hochladen" disabled="disabled"/> </div> <input type="hidden" name="key" value=""/> </form> <?js } ?> <div class="post-rules"></div> </div> ',
    data: {
        error: null
    },
    ...
    readFiles: function (files) {
        ...
        var file = files[0];
        if (!file || !CONFIG.API.ALLOWED_UPLOAD_TYPES[file.type]) {
...
p.View.Overlay.Error = p.View.Base.extend({
    name: 'overlay-error',
    template: '<div class="overlay-content"> <h2>ZOMG, Fehler!</h2> </div> <div class="overlay-content"> <?js if(error) {?> <h2 class="warn">{error.code} &ndash; {error.msg}</h2> <?js if(error.error == \'limitReached\' ) { ?> <p>Was immer du getan hast, es war zu viel. Probier\'s spÃ¤ter nochmal.</p> <?js } ?> <?js } else {?> <h2 class="warn">Irgendwas Doofes ist passiert. Probier\'s spÃ¤ter nochmal :/</h2> <?js } ?> </div> ',
    data: {
        error: null
    },
...
p.View.Layout = p.View.Base.extend({
    template: '<?js if( !p.mobile ) { ?> <div class="side-wide-skyscraper gpt" id="gpt-skyscraper-right" data-size="160x600" data-slot="pr0gramm-skyscraper"></div> <?js } else { ?> <div class="bottom-mobile-468-60" id="gpt-mobile-bottom-wrap"> <span class="close-button-wrap" data-target="#gpt-mobile-bottom-wrap"><span class="close-button">X</span></span> <div class="gpt" id="gpt-mobile-bottom" data-size="468x60" data-slot="pr0gramm-mobile"></div> </div> <?js } ?> <div id="page"> <div id="head"> <div id="head-content"> <div class="user-info not-logged-in"> <span class="link" id="login-link">anmelden</span> &nbsp; <span class="link" id="register-link">registrieren</span> </div> <div class="user-info logged-in"> <a href="#user/{user.name}" class="user-name">{user.name}</a> <a href="#inbox/unread" id="inboxLink" class="empty" title="Nachrichten"> <span class="pict">m</span> <span id="inboxCount">0</span> </a> <span class="link pict" id="settings-link" title="Einstellungen">#</span> </div> <a href="#"><img id="pr0gramm-logo" src="/media/pr0gramm.png"/></a> <div class="head-menu"> <a id="tab-new" href="#new">neu</a> <a id="tab-top" href="#top">beliebt</a> <a id="filter-link" class="logged-in" href="">{currentFilterName}</a> <span class="not-logged-in" id="search-form-spacer"></span> <form action="" id="search-form"> <input type="text" id="q" value="" title="Suche [k]"/> <input type="submit" id="search-submit" class="pict" title="Suche [k]" value="q"/> </form> <span class="link pict logged-in" id="upload-link" title="Bilder hochladen">u</span> </div> <div id="filter-menu"> <a class="filter-setting" data-flag="{CONFIG.SFW_FLAG.SFW.flag}" href=""> <div class="filter-name">sfw</div> <div class="filter-desc"> <em>Safe for work</em> Inhalt anzeigen. </div> </a> <a class="filter-setting" data-flag="{CONFIG.SFW_FLAG.NSFW.flag}" href=""> <div class="filter-name">nsfw</div> <div class="filter-desc"> Zeige <em>not safe for work</em> Inhalt. Beinhaltet Titten, Nacktheit, Pornos und leicht offensiven Kram. </div> </a> <a class="filter-setting" data-flag="{CONFIG.SFW_FLAG.NSFL.flag}" href=""> <div class="filter-name">nsfl</div> <div class="filter-desc"> Zeige <em>not safe for life</em> Inhalt. Beinhaltet... alles. Gewalt, zerplatzte Menschen, ekligen ScheiÃŸ. </div> </a> <a id="filter-save">Speichern</a> </div> </div> </div> <div id="main-view"></div> </div> <div id="overlay"> <div id="overlay-box"></div> </div> <div id="footer-links"> <a href="#faq">faq</a> <a href="#contact">kontakt</a> </div> ',
    minThumbsPerRow: 4,
    maxThumbsPerRow: 24,
    sizes: [524, 656, 788],
    data: {
    ...
    onError: function (response) {
        if (response.responseJSON && response.responseJSON.code == 404) {
            ...
            if (response.responseJSON && response.responseJSON.code == 403 && response.responseJSON.error == 'forbidden') {
                p.user.softLogout();
                this.unsetUserStatus();
                return false;
            }
            return this.showOverlay(p.View.Overlay.Error, response.responseJSON);
        }
    },
    unsetUserStatus: function () {
        $('#page').removeClass('status-admin status-user');
    },
    search: function (q) {
        var m = null;
    ...
    show: function (params) {
        ...
        this.parent(params);
        $('#page, #head').css('width', this.contentWidth);
        ...
        $('#register-link').click(this.showRegister.bind(this));
    ...
    setTab: function (tab) {
        if (tab == 'new') {
            ...
            $('#q').attr('placeholder', 'Alles durchsuchen');
        } else if (tab == 'top') {
            $('#tab-top').addClass('active');
            $('#tab-new').removeClass('active');
            $('#q').attr('placeholder', 'Beliebt durchsuchen');
        } else {
    ...
    showUpload: function () {
        return this.showOverlay(p.View.Overlay.Upload);
    },
    showResetPassword: function () {
        return this.showOverlay(p.View.Overlay.ResetPassword);
    },
    showRegister: function () {
        return this.showOverlay(p.View.Overlay.Register);
    },
    showSettings: function () {
        return this.showOverlay(p.View.Overlay.Settings);
    },
    showLogin: function () {
...
(function ($) {
    ...
    $.fn.highlight = function (r, g, b, a, wait) {
        var that = this;
...
p.View.Overlay.DeleteComment = p.View.Base.extend({
    ...
    template: '<div class="overlay-content"> <h2>Kommentar lÃ¶schen</h2> </div> <div class="overlay-content"> <form id="comment-delete-form"> <div class="form-row"> <input type="text" class="wide" name="reason" placeholder="Grund"/> </div> <div class="form-row"> <input type="hidden" name="id" value="{commentId}"/> <input type="submit" value="Durch [gelÃ¶scht] ersetzen" id="comment-delete-soft"/> <input type="submit" value="Hart LÃ¶schen (inkl. aller antworten)" id="comment-delete-hard"/> </div> </form> </div> ',
    data: {
        commentId: 0
    },
...
p.View.Stream.Comments = p.View.Base.extend({
    ...
    render: function () {
        ...
        this.$container.find('.comment-reply-link').click(this.showReplyForm.bind(this));
        this.$container.find('.comment-edit-link').click(this.showEditForm.bind(this));
        var that = this;
        this.$container.find('.vote-up').click(function (ev) {
            return that.vote(ev, 1);
        });
        this.$container.find('.vote-down').click(function (ev) {
            return that.vote(ev, -1);
        });
        this.$container.find('.comment-delete').click(function () {
...
p.View.Overlay.EditTags = p.View.Base.extend({
    ...
    render: function () {
        this.parent();
        this.$container.find('#edit-tags-invert').click(this.invertSelection.bind(this));
...
p.View.Stream.Tags = p.View.Base.extend({
    ...
    loaded: function (item) {
        ...
        this.data.tags.sort(p.View.Stream.Tags.SortConfidence);
        this.render();
    },
    render: function () {
        this.parent();
        ...
        this.$container.find('.add-tags-link').click(this.showTagForm.bind(this));
        this.$container.find('.tag-form .cancel').click(this.hideTagForm.bind(this));
        var that = this;
        this.$container.find('span.vote-up').click(function (ev) {
            return that.vote(ev, 1);
        });
        this.$container.find('span.vote-down').click(function (ev) {
            return that.vote(ev, -1);
        });
        this.$container.find('.tags-expand').click(function () {
            $(this).hide();
            $('.tag-bad').css('display', 'inline-block');
        })
        this.$container.find('#item-edit-tags').click(function () {
    ...
    vote: function (ev, vote) {
        ...
        for (var i = 0; i < this.data.tags.length; i++) {
            if (this.data.tags[i].id == tagId) {
                p.adjustVote(this.data.tags[i], absoluteVote);
                break;
            }
        }
...
p.View.Overlay.DeleteItem = p.View.Base.extend({
    ...
    template: '<div class="overlay-content"> <h2>Bild lÃ¶schen</h2> </div> <div class="overlay-content"> <form id="image-delete-form"> <div class="form-row"> <input type="text" class="wide" name="reason" placeholder="Grund"/> </div> <div class="form-row"> <input type="checkbox" name="notifyUser" checked="checked" id="delete-comment-notify"/> <label class="checkbox-label" for="delete-comment-notify">Benutzer benachrichtigen</label> </div> <div class="form-row"> <input type="checkbox" name="banUser" id="delete-ban-user"/> <label class="checkbox-label" for="delete-ban-user">Benutzer Sperren fÃ¼r</label> <input type="text" name="days" class="number" value="1"/> Tage (0 = fÃ¼r immer) </div> <div class="form-row"> </div> <div class="form-row"> <input type="hidden" name="id" value="{itemId}"/> <input type="submit" value="LÃ¶schen" id="image-delete"/> </div> </form> </div> ',
    data: {
        itemId: 0
    },
    init: function (container, parent, $el) {
...
try {
    ...
    if (p.mobile || !probe || !probe.canPlayType || !probe.canPlayType('video/webm; codecs="vp8"')) {
        ...
        script.async = true;
        script.src = '/frontend/lib/jsmpeg.min.js';
        document.body.appendChild(script);
    }
} catch (error) {};
p.View.Stream = p.View.Stream || {};
p.View.Stream.Item = p.View.Base.extend({
    template: '<div class="item-pointer"> </div> <div class="item-container-content"> <div class="item-image-wrapper"> <?js if( item.video ) { ?> <?js if( canPlayWebM ) { ?> <video class="item-image" src="{item.image}" type="video/webm" autoplay loop></video> <div class="video-position-bar"> <div class="video-position"></div> </div> <?js } else { ?> <canvas class="item-image"></canvas> <p class="no-webm"> Dein Brauser ist unfÃ¤hig WebM (korrekt?) abzuspielen. Hier ist ein runterskaliertes MPEG1. </p> <?js } ?> <?js } else { ?> <img class="item-image" src="{item.image}"/> <?js if(item.fullsize) { ?> <a href="{item.fullsize}" target="_blank" class="item-fullsize-link">+</a> <?js } ?> <?js } ?> </div> <div class="item-info"> <div class="item-vote{p.voteClass(item.vote)}"> <span class="pict vote-up">+</span> <span class="pict vote-down">-</span> <span class="score" title="{item.up} up, {item.down} down"><?js print(item.up - item.down)?></span> </div> <div class="item-details"> <?js if(item.user != p.user.name ) {?> <span class="pict vote-fav{p.favClass(item.vote)}">*</span> <?js } ?> <a class="time" title="{item.time.readableTime()}" href="/new/{item.id}">{item.time.relativeTime(true)}</a> <span class="time">von</span> <a href="#user/{item.user}" class="user um{item.mark}">{item.user}</a> <span class="item-source"> <?js if( item.source ) {?> <span class="pict">s</span>&nbsp;<a href="{{item.source}}" target="_blank">{{item.source.hostName()}}</a> <?js } else { ?> <span class="pict">s</span>upload</span> <?js } ?> </span> <?js if( !item.video ) {?> <span class="item-google-search"> <span class="pict">g</span>&nbsp;<a href="https://www.google.com/searchbyimage?hl=en&amp;safe=off&amp;site=search&amp;image_url={item.image}" target="_blank"> Bild googeln </a> </span> <?js } ?> <?js if( p.user.admin ) { ?> [<span class="action" id="item-delete" data-id="{item.id}">del</span>] [<a href="/new/phash.{item.id}.12">phash</a>] <span class="flags flags-{item.flags}">{p.Stream.FLAG_NAME[item.flags]}</span> <?js } ?> </div> <div class="item-tags"></div> </div> <div class="item-comments"></div> </div> ',
    swipeDistance: 150,
    heightKnown: false,
    init: function (container, parent) {
        this.stream = parent.stream;
    ...
    show: function (rowIndex, itemData, defaultHeight, jumpToComment) {
        ...
        var offset = rowIndex * (128 + 8) + 64 - 8;
        this.$container.find('.item-pointer').css('left', offset);
        this.$image.click(this.parentView.hideItem.bind(this.parentView));
        ...
        this.$itemVote.find('.vote-up').click(function (ev) {
            return that.vote(ev, 1);
        });
        this.$itemVote.find('.vote-down').click(function (ev) {
            return that.vote(ev, -1);
        });
        this.$itemFav = this.$container.find('.vote-fav')
        this.$itemFav.click(function (ev) {
            return that.fav(ev);
        });
    ...
    imageLoaded: function (ev) {
        ...
        if (this.video) {
            ...
            this.$videoPositionBar.click(this.jumpToPosition.bind(this));
        }
    ...
    fav: function (ev) {
        ...
        p.adjustVote(cachedItem, absoluteVote);
...
p.View.Stream.Main = p.View.Base.extend({
    baseURL: '',
    template: ' <div id="stream"></div> <div class="clear"></div> <a href="" id="stream-prev" class="pict">&lt;</a> <a href="" id="stream-next" class="pict">&gt;</a> ',
    ...
    init: function (container, parent) {
        ...
        for (var i = 1; availableWidth >= i * 128 + (i - 1) * 4; i++) {
            this.itemsPerRow = i;
        }
    ...
    prepareThumbsForInsertion: function (html) {
        var thumbs = $(html);
        thumbs.find('a').click(this.handleThumbLinkBound);
        return thumbs;
    },
    handleThumbLink: function (ev) {
        ...
        ev.preventDefault();
        this.showItem($(ev.currentTarget));
        if (!this._wasHidden) {
    ...
    loaded: function (items, position, error) {
        ...
        if (!items || !items.length) {
            var msg = null;
            if (error == 'nsfwRequired') {
                msg = 'Das Bild wurde als <em>NSFW</em> markiert.<br/>' + 'Ã„ndere deine Filter-Einstellung, wenn du es sehen willst.';
            } else if (error == 'nsflRequired') {
                msg = 'Das Bild wurde als <em>NSFL</em> markiert.<br/>' + 'Ã„ndere deine Filter-Einstellung wenn du es sehen willst.';
        ...
        // Manipulation start
        var _prefix = $('.user-name').text();
        p.View.Stream.Main.prototype.seenItems = JSON.parse(localStorage.getItem(_prefix + '_seen_items'))
        var filteredItems = [];
        for (var i = 0; i < items.length; i++) {
            if (p.View.Stream.Main.prototype.seenItems.indexOf(String(items[i].id)) > 0) {
                continue;
            }
    ...
    showItem: function ($item, scrollToFullView) {
        ...
        // Manipulation start
        var _prefix = $('.user-name').text();
        p.View.Stream.Main.prototype.seenItems.push(id);
        localStorage.setItem(_prefix + '_seen_items', JSON.stringify(p.View.Stream.Main.prototype.seenItems));
        // Manipulation end
    ...
    hideItem: function (ev) {
        ...
        this.currentItemSubview.remove();
        this.itemPrevLink.hide();
        this.itemNextLink.hide();
    },
...
p.View.User = p.View.Base.extend({
    template: '<h1 class="pane-head user-head"> {user.name} </h1> <?js if(p.user.admin) {?> <a href="http://admin.pr0gramm.com/userstats/{user.id}/" id="user-admin">admin</a> <span class="action" id="user-ban">ban</span> <?js } ?> <div class="user-stats"> <div class="user-score-pane">BENIS <span class="user-score">{user.score}</span></div> <div> <?js if(user.score < 0 ) {?>Geschrumpft<?js } else {?>Gewachsen<?js } ?> seit {user.registered.readableDate()} ({user.registered.relativeTime(true,true)}) </div> </div> <div class="tab-bar"> <span>Bilder</span> <em>{uploadCount}</em> <span>Favoriten</span> <em>{likeCount}</em> <span>Kommentare</span> <em>{commentCount}</em> <span>Tags</span> <em>{tagCount}</em> <?js if(user.mark){ ?> <em class="user-mark um{user.mark}">{p.User.MARK[user.mark]}</em> <?js } ?> </div> <?js if( p.user.name !== user.name ) { ?> <div class="pane"> <h3><span>{user.name}</span> eine private Nachricht senden</h3> <form class="message-form" method="post"> <textarea class="comment" name="comment"></textarea> <input type="hidden" name="recipientId" value="{user.id}"/> <div> <input type="submit" value="Nachricht senden"/> </div> </form> </div> <?js } ?> <div class="pane"> <h2 class="section"> Neuste Bilder <?js if( uploads.length ) { ?> <a href="#user/{user.name}/uploads" class="user-show-all">Alle Bilder Â»</a> <?js } ?> </h2> <?js if( !uploads.length) {?> <span class="disabled">Bisher keine Bilder</span> <?js } ?> </div> <?js if( uploads.length) {?> <div class="stream-row in-pane"> <?js for( var i = 0; i < uploads.length; i++ ) { var u = uploads[i]; ?> <a class="thumb" href="#user/{user.name}/uploads/{u.id}"><img src="{u.thumb}"/></a> <?js } ?> <div class="clear"></div> </div> <?js } ?> <div class="pane"> <h2 class="section"> Neuste Favoriten <?js if( likesArePublic && likes.length ) { ?> <a href="#user/{user.name}/likes" class="user-show-all">Alle Favoriten Â»</a> <?js } ?> </h2> <?js if(!likesArePublic) { ?> <span class="disabled">Nicht Ã¶ffentlich</span> <?js } else if( !likes.length) { ?> <span class="disabled">Bisher keine Favoriten</span> <?js } ?> </div> <?js if(likesArePublic && likes.length) {?> <div class="stream-row in-pane"> <?js for( var i = 0; i < likes.length; i++ ) { var l = likes[i]; ?> <a class="thumb" href="#user/{user.name}/likes/{l.id}"><img src="{l.thumb}"/></a> <?js } ?> <div class="clear"></div> </div> <?js } ?> <div class="pane"> <h2 class="section"> Neuste Kommentare <?js if( comments.length ) { ?> <a href="#user/{user.name}/comments/before/{newestCommentTime}" class="user-show-all"> Alle Kommentare Â» </a> <?js } ?> </h2> <?js for( var i = 0; i < comments.length; i++ ) { var c = comments[i]; ?> <div class="comment"> <a href="#new/{c.itemId}:comment{c.id}"> <img src="{c.thumb}" class="comment-thumb"/> </a> <div class="comment-content with-thumb"> {c.content.format()} </div> <div class="comment-foot with-thumb"> <a href="#user/{user.name}" class="user um{user.mark}">{user.name}</a> <span class="score">{"Punkt".inflect(c.score)}</span> <a href="#new/{c.itemId}:comment{c.id}" class="time permalink" title="{c.created.readableTime()}">{c.created.relativeTime(true)}</a> </div> </div> <?js } ?> <?js if( !comments.length) {?> <span class="disabled">Bisher keine Kommentare</span> <?js } ?> </div>',
    init: function (container, parent) {
        this.parent(container, parent);
        p.mainView.setTab(null);
    },
    load: function () {
        ...
        if (p.user.name && p.user.name === this.data.params.name) {
            opts.self = true;
        }
        p.api.get('user.info', opts, this.loaded.bind(this));
        return false;
    },
    render: function () {
        this.parent();
        ...
        this.$container.find('#user-ban').click(function () {
...
p.View.User.Comments = p.View.Base.extend({
    ...
    load: function () {
        ...
        p.api.get('user.comments', opts, this.loaded.bind(this));
        return false;
    },
    loaded: function (response) {
        this.data.user = response.user;
...
p.View.Inbox = p.View.Base.extend({
    ...
    render: function () {
        ...
        this.$commentForm.find('input.cancel').hide().click(this.cancelReply.bind(this));
        this.$container.find('.comment-reply-link').click(this.showReplyForm.bind(this));
        this.$container.find('.expand-thread').click(function () {
            $(this).hide();
...
p.View.Validate = p.View.Base.extend({
    template: '<h2 class="main-message">UngÃ¼ltiges Token</h2>',
    load: function () {
...
p.View.ResetPassword = p.View.Base.extend({
    template: ' <?js if(submitted && !error ) {?> <h2 class="main-message">Passwort gespeichert!</h2> <?js } else { ?> <h1 class="pane-head">Passwort fÃ¼r {{params.name}} setzen</h1> <div class="pane"> <form id="reset-password"> <div class="form-row"> <input type="password" value="" placeholder="Neues Passwort" name="password"/> <?js if( error === \'invalidToken\' ) {?> <p class="warn">UngÃ¼ltiger oder veralteter Token.</p> <?js } else if( error === \'passwordTooShort\' ) { ?> <p class="warn">Passwort zu kurz</p> <?js } ?> <p class="field-details">Mindestens 6 Zeichen</p> </div> <div class="form-row"> <input type="submit" value="Speichern"/> </div> <input type="hidden" name="token" value="{{params.token}}"/> <input type="hidden" name="name" value="{{params.name}}"/> </form> </div> <?js } ?>',
    data: {
        submitted: false,
        error: null
    },
...
p.View.Contact = p.View.Base.extend({
    template: '<h1 class="pane-head">Kontakt</h1> <div class="pane"> <?js if(sent) { ?> <p>Deine Nachricht wurde verschickt!</p> <?js } else { ?> <p> FÃ¼r LÃ¶schanfragen bitte die exakte Adresse (URL), sowie ausreichende Beweise angeben, dass Du der Urheber des Bildes bist oder in seinem Auftrag handelst. </p> <form class="contact-form" method="post"> <div class="form-row"> <input type="email" name="email" placeholder="E-Mail Adresse"/> </div> <textarea name="message" placeholder="Nachricht"></textarea> <div> <input type="submit" value="Abschicken"/> </div> </form> <?js } ?> </div>',
    data: {
        sent: false
    },
    render: function () {
...
p.View.FAQ = p.View.Base.extend({
    template: '<h1 class="pane-head">FAQ - HÃ¤ufig gestellte Fragen</h1> <div class="pane"> <h2 class="section">Gibt es hier Regeln?</h2> <p>Ja.</p> <div class="faq-rules"></div> <h2 class="section">Wieso ist mein Benis so kurz?</h2> <p> Der Benis ist Summe aller Votes die, Du erhalten hast. Dein Benis verlÃ¤ngert oder verkÃ¼rzt sich mit jedem positiven oder negativen Vote, den Deine Kommentare, Tags oder Bilder erhalten. </p> <p> Wenn Dein Benis zu kurz ist, solltest Du aufhÃ¶ren so viel ScheiÃŸe zu posten. </p> <h2 class="section">Was ist <em>403 â€“ Limit Reached</em>?</h2> <p> Alle Aktionen (Kommentare oder Tags schreiben, Bilder hochladen etc.) haben ein bestimmtes Limit. Wenn dieses Limit erreicht ist, musst Du eine Weile warten, bis Du die Aktion nochmal ausfÃ¼hren darfst. </p> <p> Kommentare, Private Nachrichten, Tags und Votes sind stÃ¼ndlich limitiert. </p> <p> Das Limit fÃ¼r Bilder gilt fÃ¼r ca. einen Tag und ist von der BenislÃ¤nge abhÃ¤ngig: Bei Microbenis 1 Bild, ab 100 Benis 3 Bilder, ab 1000 Benis 5 Bilder. </p> <h2 class="section">Wie kann ich ein Bild von mir lÃ¶schen?</h2> <p> Wenn es Deine Schulfreunde nicht so lustig fanden, auf pr0gramm gelandet zu sein wie Du dachtest, oder es sonstige triftige GrÃ¼nde fÃ¼r eine LÃ¶schung gibt, kannst Du uns Ã¼ber das <a href="#contact">Kontaktformular</a> eine LÃ¶schanfrage schicken. </p> <p> Wenn Du ein Bild gelÃ¶scht haben mÃ¶chtest, nur weil es viele Downvotes erhÃ¤lt, hast Du Pech gehabt. Vielleicht solltest Du aufhÃ¶ren so viel ScheiÃŸe zu posten. </p> <h2 class="section">Wieso bekomme ich keine Registrierungsmail?</h2> <p> Um es kurz zu machen: Wir haben die Einrichtung unseres Mailservers verkackt. Viele Registrierungsmails landeten im Spam oder wurden von Eurem Mail-Provider komplett abgewiesen. </p> <p> Mittlerweile sollte das besser funktionieren. Nicht aktivierte Benutzernamen werden nach einem Tag gelÃ¶scht, sodass Du es einfach nochmal probieren kannst. </p> <p> Generell machen <strong>Yahoo</strong>, <strong>Freenet</strong> und <strong>GMX</strong> viele Mucken bei unseren Mails. Wenn ihr immer noch keine Mail bekommen habt, probiert <a href="https://mail.google.com">GMail</a>. </p> <h2 class="section">Was bedeuten die Farbigen Punkte hinter den Benutzernamen?</h2> <p> Die farbigen Punkte geben den Status an. Welcher das genau ist, steht auf der Profilseite des jeweiligen Benutzers. </p> <p> Neuschwuchteln, die einen langen Benis haben, wird nach einer Weile der Neuschwuchtelstatus abgenommen. </p> <h2 class="section">Welche Keyboard-Shortcuts gibt es?</h2> <ul> <li><strong>Pfeiltasten</strong> â€“ NÃ¤chstes/vorheriges Bild</li> <li><strong>K</strong> â€“ Zur Suche springen</li> <li><strong>T</strong> â€“ Zu den Tags springen</li> <li><strong>C</strong> â€“ Zu den Kommentaren springen</li> <li><strong>W/S</strong> â€“ Bild hoch-/runter-voten</li> <li><strong>F</strong> â€“ Bild favorisieren</li> </ul> <h2 class="section">Warum ist das neue Design so scheiÃŸe, Du Sohn einer blutenden Hafenhure?</h2> <p> Deine Mutter hat mir dabei geholfen :/ </p> </div> ',
    rulesView: null,
    render: function () {
        this.parent();
...
if (!p.getLocation().match(/^bm\//)) {
    ...
    p.addRoute(p.View.Stream.Main, '<tab:top|new>');
    p.addRoute(p.View.Stream.Main, '<tab:top|new>/<itemId:d>');
...
if (CONFIG.ANALYTICS.ENABLED) {
    ...
    _gaq.push(['_trackPageview', p.getLocation()]);
